...
{
public:
  Renderer() : m_screenBuffer(0x30000, 128) {}
  ...
  std::vector<uint8_t> &screenBuffer() { return m_screenBuffer; }

  uint32_t getMainTextureID() const { return mainwindowTextureId; }

  uint64_t getUpdateTick() const { return updateTick; }
