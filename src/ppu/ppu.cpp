...
bool ppu_is_odd_cycle() { return (PPU_cycles & 1) == 1; }

uint ppu_get_cycle() { return PPU_cycles; }

uint ppu_get_current_scanline_cycle() { return s_arMod341[PPU_cycles]; }
...
static void ppu_process_visible_frame(const uint &ppu_cycle)
{
  if (is_rendering_enabled()) {
    ...
    if (ppu_cycle >= 1 && ppu_cycle <= 256) {
      ...
      if (is_bg_enabled()) { ppu_bg_draw(ppu_cycle); }
      ppu_sprite_get_active();
      if (is_sprite_enabled()) { ppu_sprite_draw(ppu_cycle); }
...
void ppu_do_cycle()
{
  ...
  if (auto scanline = PPURegs().scanline; scanline <= 239U) {
    ...
    if (ppu_cycle == 1) {
      ...
      */
      if (PPURegs().last_2002_read == PPU_cycles /* || (PPURegs().last_2002_read + 1) == PPU_cycles*/) {
        ...
        set_vblank();
        if (GMemoryRegisters().r2000 & 128) { GRegisters().nmi = true; }
      }
    }
    ...
    if (ppu_cycle == 339) {
      ...
      ppu_process_prerender_frame(ppu_cycle);
      if (ppu_cycle == 340) { UpdateTextureFromPPU(); }
    }
  }
