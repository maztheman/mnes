...
inline uint8_t ppu_memory_main_read_byte(uint address) { return TO_BYTE(ppu_memory_main_read(address)); }
...
inline void ppu_bg_draw(const uint &ppu_cycle)
{
  ...
  if (ppu_cycle < 9 && show_bg_leftmost_8px() == false) { bDraw = false; }
