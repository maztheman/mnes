...
struct ppu_sprite_state_t
{
  ...
  {
    oam_2nd_index++;
    if (oam_2nd_index == 8 && process_type == 1) { process_type = 2; }
  }
  ...
  {
    if (++oam_m_index >= 4) { oam_m_index = 0; }
  }
...
static inline void ppu_sprite_initalize(const uint &ppu_cycle)
{
  ...
  if (ppu_cycle == 1) { GMemoryRegisters().oam_clear_reads = true; }
...
static inline void ppu_sprite_process_1()
{
  ...
  if (is_sprite_in_range(ppu_scanline(), sprite_state.oam_latch[0])) {
    ...
    if (sprite_state.oam_index == 0) { sprite_state.sprite0_in_range = true; }
...
static inline void ppu_sprite_process_3() { sprite_state.inc_oam_index(); }
...
static inline void ppu_sprite_fetch_sprite_data(const uint &ppu_cycle)
{
  ...
  *
  *
  In addition to this, the X positions and attributes for each sprite are loaded from the secondary OAM into their
  respective counters/latches. This happens during the second garbage nametable fetch, with the attribute byte loaded
  during the first tick and the X coordinate during the second. Garbage nametable fetch: 0 - 1 Garbase nametable fetch:
  2 - 3
  ...
  if (sprite_cycle == 0) {
    ...
    if (selected_index >= sprite_state.oam_2nd_index) {
      ...
      MLOG_PPU(" N:$%04X SL:%ld PC:$%04X C:%ld H:%ld T:[$%016lX]\n",
        GMemoryRegisters().ppu_addr_bus,
        ppu_scanline(),
        GRegisters().pc,
        ppu_get_current_scanline_cycle(),
        height,
    ...
    if (selected_index < sprite_state.oam_2nd_index) {
      MLOG_PPU("Sprite C6: A:$%04X SL:%ld PC:$%04X C:%ld H:%ld\n",
        ...
        GRegisters().pc,
        ppu_get_current_scanline_cycle(),
        ((GMemoryRegisters().r2000 & 0x20) ? 16 : 8))
    }
    ...
    if (selected_index >= sprite_state.oam_2nd_index) {
      ...
      if (GMemoryRegisters().ppu_addr_bus != pattern_table_addr) {
        MLOG_PPU("Sprite Pattern table is not the same PPU_addr:$%04X vs $%04X",
          GMemoryRegisters().ppu_addr_bus,
          pattern_table_addr)
      }
...
{
  if (ppu_cycle < 9 && show_sprite_leftmost_8px() == false) { return; }

  if (ppu_scanline() == 0) { return; }
  ...
  for (int i = 0; i < 8; i++) {
    if (sprite_active[i]) {
      ...
      if (i == 0 && /*sprite_state.sprite0_in_range &&*/ is_bg_enabled()) {// redundant check for bg enabled, bgcolor
                                                                           // array should be 0.. could be sprite 0 hit
        if (g_aBGColor[RealX] == 1 && chrcolor != 0 && is_sprite_zero_hit() == false) {
          if (RealX != 255) { set_sprite_zero_hit(); }
        }
      }
