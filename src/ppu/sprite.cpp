...
void copy_to_secondary(const uint32_t &ppu_cycle)
{
  bool even_cycle = (ppu_cycle & 1) == 0 ? true : false;
  if (ppu_cycle == 65) {
    cmem::set_oam_clear_reads(false);
    sprite_state.reset();
  }
...
void fetch_sprite_data(const uint32_t &ppu_cycle)
{
  ...
  */
  ...
  if (sprite_cycle == 0) {
	...
	//we need to be cycle accurate for the cmem::ppu_addr_bus()
...
void draw(const uint32_t &ppu_cycle)
{
  ...
  for (int i = 0; i < 8; i++) {
    if (sprite_active[i]) {
      ...
      if (i == 0
          && /*sprite_state.sprite0_in_range &&*/ cmem::is_bg_enabled()) {// redundant check for bg enabled, bgcolor
        // array should be 0.. could be sprite 0 hit
        if (g_aBGColor[RealX] == 1 && chrcolor != 0 && cmem::is_sprite_zero_hit() == false) {
          if (RealX != 255) { cmem::set_sprite_zero_hit(); }
        }
      }
