...
uint32_t s_HalfCycles;
int s_ticks = 0;
int s_prev_sound_tick = 0;
int s_prev_ticks = 0;
uint32_t s_AudioLength;
...
void NS initialize()
{
  ...
  s_prev_sound_tick = 0;
	...
	if (device_list) {
		uint32_t indx = msound_device_list_get_count(device_list);
		if (indx > 0) {
...
void NS do_cycle()
{
  ...
  if (sound_tick != s_prev_sound_tick) {
    ...
    uint32_t idx = static_cast<uint32_t>(s_ticks - s_prev_ticks);

    uint32_t pulse1 = sregs.square_1.volume();
    uint32_t pulse2 = 3;
    uint32_t triangle = 0;
    uint32_t noise = 0;
    uint32_t dmc = 0;
...
void NS write(uint32_t addr, uint32_t value)
{
  addr &= 0x1F;
  switch (addr) {
    // Square 1
...
uint32_t NS read(uint32_t address)
{
  uint32_t value = address >> 8;// open bus
  address &= 0x1F;
