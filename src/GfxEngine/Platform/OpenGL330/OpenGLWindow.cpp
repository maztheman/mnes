...
{
  // because the window events are already hooked up all we must do is set the member here and the plumbing will take
  // care of rest
  m_Data.onWindowCloseFn = onClose;
}

void OpenGLWindow::SetOnKeyEvent(OnKeyEvent onKeyEvent) { m_Data.onKeyEvent = onKeyEvent; }
