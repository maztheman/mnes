...
static inline void cpu_brk()
{
  ...
  if (GRegisters().actual_irq == doing_irq::brk) { GRegisters().pc++; }
  // 3-4
  memory_push_pc();
...
#ifdef USE_LOG
    if (GRegisters().actual_irq == doing_irq::brk) { VLog().AddLine("** IRQ INTERRUPTED BRK **\n"); }
#endif
  }
  // 5
  memory_push_byte((GRegisters().actual_irq != doing_irq::brk) ? (GRegisters().status | BFLAG_10_MASK)
                                                               : (GRegisters().status | BFLAG_10_MASK | BFLAG_01_MASK));
  // 6
