...
template<typename T> static uint TO_ZERO_PAGE(T address) { return address & 0xFF; }
...
sram_memory_t SRam() { return sram_memory_t(MemoryData().Memory().subspan(SRAM_OFFSET, SRAM_SIZE)); }

spr_ram_t SPRRam() { return spr_ram_t(MemoryData().Memory().subspan(SPR_RAM_OFFSET, SPR_RAM_SIZE)); }

tmp_spr_ram_t TmpSPRRam() { return tmp_spr_ram_t(MemoryData().Memory().subspan(TMP_SPR_RAM_OFFSET, TMP_SPR_RAM_SIZE)); }
...
uint memory_pc_peek(uint address) { return current_mapper()->read_memory(address); }

uint memory_main_read(uint address)
{
  uint nMapperAnswer = current_mapper()->read_memory(address);

  if (address < 0x2000) { return MainMemory()[address & 0x7FF]; }
  ...
  if (address < 0x6000) {
    ...
    if (original == 0x4015) { return apu_memory_read(original); }
    // joypad
    switch (address) {
    case 0x14:// w
    case 0x15:// w
...
void memory_main_write(uint address, uint value)
{
  ...
  if (address < 0x6000) {
    ...
    case 0x14:// w
    {
      ...
      for (uint i = 0; i < 256; i++) { ext_memory_write(0x2004, ext_memory_read(nStartAddress + i)); }
...
static inline void check_for_software_irq()
{
  if (GRegisters().actual_irq == doing_irq::none) {
    if (GRegisters().opCode == mnes::opcodes::OPCODE_BRK) { GRegisters().actual_irq = doing_irq::brk; }
  }
}
